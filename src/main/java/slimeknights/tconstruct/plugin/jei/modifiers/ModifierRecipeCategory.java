...
public class ModifierRecipeCategory implements IRecipeCategory<IDisplayModifierRecipe> {
  ...
  // translation
  private static final List<Component> TEXT_FREE = Collections.singletonList(Util.makeTranslation("jei", "modifiers.free"));
  private static final List<Component> TEXT_SINGLE_UPGRADE = Collections.singletonList(Util.makeTranslation("jei", "modifiers.upgrade"));
  private static final List<Component> TEXT_INCREMENTAL = Collections.singletonList(Util.makeTranslation("jei", "modifiers.incremental"));
  private static final String KEY_UPGRADES = Util.makeTranslationKey("jei", "modifiers.upgrades");
  private static final List<Component> TEXT_SINGLE_ABILITY = Collections.singletonList(Util.makeTranslation("jei", "modifiers.ability"));
  ...
  public void draw(IDisplayModifierRecipe recipe, PoseStack matrices, double mouseX, double mouseY) {
    ...
    if (text != null) {
      int x = 112 - fontRenderer.width(text);
  ...
  public List<Component> getTooltipStrings(IDisplayModifierRecipe recipe, double mouseX, double mouseY) {
    ...
    if (recipe.hasRequirements() && GuiUtil.isHovered(checkX, checkY, 66, 58, 16, 16)) {
      ...
      if (abilities > 0) {
        return abilities == 1 ? TEXT_SINGLE_ABILITY : Collections.singletonList(new TranslatableComponent(KEY_ABILITIES, abilities));
      } else if (upgrades > 0) {
        return upgrades == 1 ? TEXT_SINGLE_UPGRADE : Collections.singletonList(new TranslatableComponent(KEY_UPGRADES, upgrades));
      } else {
        return TEXT_FREE;
      }
    }
