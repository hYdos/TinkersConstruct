...
public class CastingItemRenderTypeBuffer implements MultiBufferSource {
  ...
  public VertexConsumer getBuffer(RenderType type) {
    if (alpha < 255 && MAKE_TRANSPARENT.contains(type.name) && type instanceof RenderType.CompositeRenderType) {
      ResourceLocation texture = ((RenderType.CompositeRenderType) type).state.textureState.texture.orElse(InventoryMenu.BLOCK_ATLAS);
